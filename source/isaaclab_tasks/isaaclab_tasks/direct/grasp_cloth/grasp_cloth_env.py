...
"""Grasp-Cloth: Franka grasps a hanging cloth using the coupled solver (MuJoCo + VBD)."""
...
from .grasp_cloth_env_cfg import GraspClothEnvCfg

logger = logging.getLogger(__name__)


class GraspClothEnv(DirectRLEnv):
    """Grasp-Cloth environment: Franka + hanging cloth via coupled solver."""

    cfg: GraspClothEnvCfg

    def __init__(self, cfg: GraspClothEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        logger.info(
            "GraspClothEnv: control_mode=%s, action_scale=%s, interactive_ik=%s",
    ...
    def _setup_cloth_pinning(self):
        ...
        corner1_idx = y_edge_indices[edge_pos[:, 0].argmin()]
        corner2_idx = y_edge_indices[edge_pos[:, 0].argmax()]
    ...
    def _setup_interactive_ik(self):
        ...
        try:
            ...
            if newton_model is None:
                logger.info("[GraspClothEnv] Newton model not available; IK disabled.")
                return
            ...
            # Use a fixed pre-grasp pose (same as grasp_avbd_cloth)
            ...
            self._newton_viewer_gl = None
            logger.info("[GraspClothEnv] Newton IK initialized (EE index=%d)", self._ee_ik_index)
            ...
            logger.info("[GraspClothEnv] IK not available: %s", exc)
    ...
    def _apply_ik_action(self):
        ...
        if self._newton_viewer_gl is None:
            ...
            if self._newton_viewer_gl is not None:
                ...
                logger.info("[GraspClothEnv] Newton viewer gizmo registered")
            else:
                logger.warning("[GraspClothEnv] NewtonViewerGL not found")
    ...
    def _register_reset_key(self):
        ...
        def _on_key(symbol, modifiers, _self=self):
            if symbol == key.R:
                ...
                print("[GraspClothEnv] Reset requested via R key")
            elif symbol == key.G:
                _self._gripper_closed = not _self._gripper_closed
                print(f"[GraspClothEnv] Gripper {'closed' if _self._gripper_closed else 'open'} via G key")

        self._newton_viewer_gl.renderer.register_key_press(_on_key)
        self._reset_key_registered = True
        logger.info("[GraspClothEnv] R key (reset) and G key (gripper toggle) registered")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # Sync rigid-body state from state_0 to state_1 for reset envs.
        # VBD (with integrate_with_external_rigid_solver=True) reads body_q from state_out,
        # which is state_1 in the first substep.
