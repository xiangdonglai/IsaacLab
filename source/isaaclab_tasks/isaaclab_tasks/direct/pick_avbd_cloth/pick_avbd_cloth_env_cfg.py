...
"""Configuration for the Pick-AVBD-Cloth environment: Franka + cloth via unified AVBD solver.

Ported from the screenshots branch.  Uses the cursor branch's per-solver layout:
the physics manager is the standalone :class:`NewtonVBDManager` (selected by
``solver_cfg=VBDSolverCfg(...)``) — no coupling wrapper, since AVBD integrates
rigid bodies internally.
"""
...
# AVBD-specific Franka config: very high joint stiffness for ALM (augmented
# Lagrangian) coupling between the PD controller and the AVBD rigid solver.
...
class ClothAVBDNewtonCfg(NewtonCfg):
    """:class:`NewtonCfg` extended with model-level contact parameters.

    Distinct name ensures the ``_is_kitless_physics`` heuristic does not match
    it, so Kit is launched (required for USD deformable spawning).
    """
...
class PickAVBDClothPhysicsCfg(PresetCfg):
    """Physics presets for AVBD cloth picking (unified rigid + cloth via AVBD)."""
