...
"""Grasp-AVBD-Cloth: Franka grasps a hanging cloth using the unified AVBD solver."""
...
from .grasp_avbd_cloth_env_cfg import GraspAVBDClothEnvCfg

logger = logging.getLogger(__name__)


class GraspAVBDClothEnv(DirectRLEnv):
    """Grasp-AVBD-Cloth environment: Franka + hanging cloth via AVBD solver."""

    cfg: GraspAVBDClothEnvCfg

    def __init__(self, cfg: GraspAVBDClothEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        logger.info(
            "GraspAVBDClothEnv: control_mode=%s, action_scale=%s, interactive_ik=%s",
            self.cfg.control_mode,
            cfg.action_scale,
            self._ik_available,
        )

    def _setup_cloth_pinning(self):
        """Pin the top two corners of the cloth by setting kinematic targets.

        Uses the persistent ``nodal_kinematic_target`` buffer following the
        pattern in ``scripts/tutorials/01_assets/run_deformable_object.py``.
        """
        # Clone the persistent kinematic target buffer
        self._kinematic_target = wp.to_torch(self.cloth.data.nodal_kinematic_target).clone()

        # Get default nodal positions to find the top corners
        default_state = wp.to_torch(self.cloth.data.default_nodal_state_w)
        nodal_pos = default_state[:, :, :3]  # (num_envs, N, 3)

        # Initialize kinematic target positions from default state, all free
        self._kinematic_target[..., :3] = nodal_pos
        self._kinematic_target[..., 3] = 1.0  # All free

        # Pin two corners along X (robot's front direction).
        # The cloth is flat in XY. Pin the two x-extreme corners on the
        # positive-y edge so the pinned line is along X and the cloth
        # drapes away from the robot (toward -y) under gravity.
        pos = nodal_pos[0]  # (N, 3) for env 0
        y_max = pos[:, 1].max()
        y_edge_mask = pos[:, 1] > (y_max - 0.01)
        y_edge_indices = y_edge_mask.nonzero(as_tuple=True)[0]
        if len(y_edge_indices) < 2:
            logger.warning("Could not find edge corners for pinning")
            self._pinned_indices = []
            return

        edge_pos = pos[y_edge_indices]
        corner1_idx = y_edge_indices[edge_pos[:, 0].argmin()]  # min x
        corner2_idx = y_edge_indices[edge_pos[:, 0].argmax()]  # max x
        self._pinned_indices = [int(corner1_idx), int(corner2_idx)]

        # Pin these corners (flag=0) for all envs
        for idx in self._pinned_indices:
            self._kinematic_target[:, idx, 3] = 0.0

        # Write initial kinematic targets
        self.cloth.write_nodal_kinematic_target_to_sim_index(self._kinematic_target)

        logger.info(
            "Cloth corners pinned: idx=%s positions=%s",
            self._pinned_indices,
            pos[self._pinned_indices].cpu().numpy(),
        )

    def _apply_cloth_pinning(self):
        """Re-apply kinematic targets each step (pinned corners stay fixed)."""
        if not hasattr(self, "_pinned_indices") or not self._pinned_indices:
            return
        self.cloth.write_nodal_kinematic_target_to_sim_index(self._kinematic_target)
    ...
    def _setup_interactive_ik(self):
        ...
        try:
            ...
            if newton_model is None:
                logger.info("[GraspAVBDClothEnv] Newton model not available; IK disabled.")
                return
            ...
            self._newton_viewer_gl = None
            logger.info("[GraspAVBDClothEnv] Newton IK initialized (EE index=%d)", self._ee_ik_index)
            ...
            logger.info("[GraspAVBDClothEnv] IK not available: %s", exc)
    ...
    def _apply_ik_action(self):
        ...
        if self._newton_viewer_gl is None:
            ...
            if self._newton_viewer_gl is not None:
                ...
                logger.info("[GraspAVBDClothEnv] Newton viewer gizmo registered")
            else:
                logger.warning("[GraspAVBDClothEnv] NewtonViewerGL not found")
    ...
    def _register_reset_key(self):
        ...
        def _on_key(symbol, modifiers, _self=self):
            if symbol == key.R:
                ...
                print("[GraspAVBDClothEnv] Reset requested via R key")
            elif symbol == key.G:
                _self._gripper_closed = not _self._gripper_closed
                print(f"[GraspAVBDClothEnv] Gripper {'closed' if _self._gripper_closed else 'open'} via G key")

        self._newton_viewer_gl.renderer.register_key_press(_on_key)
        self._reset_key_registered = True
        logger.info("[GraspAVBDClothEnv] R key (reset) and G key (gripper toggle) registered")
