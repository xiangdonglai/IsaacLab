...
"""Configuration for the Grasp-AVBD-Cloth environment: Franka + hanging cloth, all via AVBD solver."""
...
@configclass
class GraspAVBDClothPhysicsCfg(PresetCfg):
    """Physics presets for AVBD cloth grasping."""
...
@configclass
class GraspAVBDClothEnvCfg(DirectRLEnvCfg):
    decimation = 2
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        physics=GraspAVBDClothPhysicsCfg(),
    ...
    cloth: DeformableObjectCfg = DeformableObjectCfg(
        ...
        init_state=DeformableObjectCfg.InitialStateCfg(
            pos=(.75, 0.0, .66),
            rot=(-math.sqrt(0.5), 0.0, 0.0, math.sqrt(0.5)),  # Identity — flat in XY, drapes down in Z
        ),
    )

    # Number of particles along each edge of the cloth grid
